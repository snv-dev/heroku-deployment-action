...
jobs:
  build:
    ...
    steps:
      ...
      - uses: snv-dev/heroku-deployment-action@v1.0.0 # This is the action
        with:
...
| buildpacks *                  | false    | An optional buildpacks to use when creating the heroku application                                                                                                                                   | buildpack-1,buildpack-2,... |
| addons *                  | false    | An optional addons to use when creating the heroku application                                                                                                                                   | addon-1,addon-2,... |
| formation *                   | false    | An optional formation to use when creating the heroku application                                                                                                                                   | web=1:basic,worker=1:basic |
...
| env_variables               | false    | custom env variables                                                                                                                                                      | ENV_1_NAME=ENV_1_VALUE,ENV_2_NAME=ENV_2_VALUE,...                          |
...
_* - normally the values for these properties are taken from the app.json file automatically but can be used manually too_
...
jobs:
  build:
    ...
    steps:
      - uses: actions/checkout@v2
      - uses: snv-dev/heroku-deployment-action@v1.0.0
        with:
          heroku_api_key: ${{secrets.HEROKU_API_KEY}}
...
- You can check this repo's [_.github/workflows/main.yml_](https://github.com/snv-dev/heroku-deployment-action/blob/master/.github/workflows/main.yml) for example use cases of the action in use. Additionally the APIs for these use cases can be found in the [_tests_](https://github.com/snv-dev/heroku-deployment-action/tree/master/tests) folder of the repo
...
This project is licensed under the MIT License - see the [LICENSE](https://github.com/snv-dev/heroku-deployment-action/blob/master/LICENSE) file for details
